...
public class TrainStation extends Application {
    ...
    private static ArrayList<Passenger> waitingList = new ArrayList<>();
    ...
    public static void viewTrainQueue() {
        ...
        //Displaying the remaining slots empty
    ...
    public static void loadData() throws IOException, ClassNotFoundException {
        ...
        for (int count = 0; waitingList.get(count) != null; count++) {
            if (trainQueue.getQueueArray()[index] != null) {
                if (trainQueue.getQueueArray()[index].getSeatNumber() == waitingList.get(fixed).getSeatNumber()) {
                    waitingList.remove(fixed);
                }
                index++;
            }
        }
    ...
    public static void simulation() throws IOException {
        ...
        for(Passenger passenger : trainQueue.getQueueArray()){
            if(passenger!=null){
                ...
                list_queue.getItems().add("\t\t"+noOfPassengers+"\t\t\t\t\t"+maxQueueTime+"\t\t\t\t\t"+minQueueTime);
                queueCount++;
